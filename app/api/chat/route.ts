...
// 简易 LLM 调用（复用 pipeline.mjs 模式）
async function callLLM(systemPrompt: string, userMsg: string, temperature: number): Promise<string | null> {
...
export async function POST(request: Request) {
  try {
    ...
    var systemPrompt = "你是一位专业的 ESG 知识顾问，名叫「ESG Compass」。回答基于用户提供的知识和当下全球 ESG 政策/标准，给出简洁、精准、实用的中文回答。回答不超过 300 字。如果知识库中没有相关信息，诚实地告诉用户你不知道并给出力所能及的一般性建议。回答末尾标注引用的知识条目名称。";
    var userPrompt = "参考信息：\n" + contextStr + "\n---\n用户问题：" + message;
    var reply = await callLLM(systemPrompt, userPrompt, 0.3);
    if (!reply) {
      var matchCount = allSources.length;
      if (eventCtx && matchCount > 0) {
        reply = "关于「" + eventCtx.title + "」的相关议题，我在知识库中找到了 " + matchCount + " 份参考资料。\n\n";
        allSources.slice(0, 2).forEach(function (kb) {
          reply += "**" + kb.title + "**：" + kb.summary + "\n\n";
        });
        reply += "建议你阅读原文获取完整内容。如果需要更具体的某一方面（如合规要求、披露指引、供应链管理等），可以继续提问。";
      } else if (matchCount > 0) {
        reply = "我找到了 " + matchCount + " 份相关的参考资料。\n\n";
        allSources.slice(0, 3).forEach(function (kb) {
          reply += "**" + kb.title + "**：" + kb.summary.substring(0, 200) + "\n\n";
        });
        reply += "请告诉我你具体想了解哪个方面，我可以进一步深入。";
      } else {
        reply = "我目前的知识库暂未收录该主题的详细信息。建议你通过首页的 ESG 快讯或知识专区浏览当前热点，或换一个关键词再试试。";
      }
    }
...
export async function GET() {
  var kb = await import("@/lib/knowledge-base");
  return NextResponse.json({ totalEntries: kb.KNOWLEDGE_BASE.length, categories: ["法规", "标准", "解读", "实践"] });
}
